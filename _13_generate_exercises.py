...
def generate_exercises(
    ...
    if stage == 0:
        system_content = f"""Ты – опытный преподаватель-методист, создающий тесты с одним правильным ответом на основе учебного материала.
Твоя задача – сгенерировать {count} упражнение(ий) уровня сложности {difficulty}.

Требования:
1. Каждый вопрос должен иметь 4-5 вариантов ответа, включая ровно один правильный ответ.
2. Правильный ответ должен однозначно следовать из текста.
3. Вопросы должны быть разнообразными и не повторять предыдущие.
4. Вопросы должны проверять понимание и верное применение именно тех концепций, которые автор материала пытается объяснить/сделать понятным. Вопрос не должен проверять запоминание фактов или примеров из текста.
5. Если вопрос опирается на цитату или пример из текста, обязательно приведите эту цитату или пример в тексте вопроса.
...
2. Правильные ответы должны однозначно следовать из текста.
3. Вопросы должны быть разнообразными и не повторять предыдущие.
4. Вопросы должны проверять понимание и верное применение именно тех концепций, которые автор материала пытается объяснить/сделать понятным. Вопрос не должен проверять запоминание фактов или примеров из текста.
5. Если вопрос опирается на цитату или пример из текста, обязательно приведите эту цитату или пример в тексте вопроса.
...
```"""
        ...
        system_content = f"""Ты – опытный преподаватель-методист, создающий открытые вопросы на проверку изученного материала.
Твоя задача – сгенерировать {count} открытый вопрос уровня сложности {difficulty}.

Требования:
1. Вопросы должны быть разнообразными и не повторять предыдущие.
2. Для вопроса предоставь модельный ответ и критерии оценки.
3. Вопросы должны проверять понимание и верное применение именно тех концепций, которые автор материала пытается объяснить/сделать понятным. Вопрос не должен проверять запоминание фактов или примеров из текста.
4. Если вопрос опирается на цитату или пример из текста, обязательно приведите эту цитату или пример в тексте вопроса.
